...
int winscap_init(struct SCConfig *config, WINSCAP_HANDLE *handle) {
    ...
    int rate = static_cast<int>(config->sampleRate);
    int bits = static_cast<int>(config->bits);

    struct WinscapImp* imp = new struct WinscapImp;
    imp->wfx.wFormatTag = WAVE_FORMAT_PCM;
    imp->wfx.nChannels = (WORD)channels;
    imp->wfx.nSamplesPerSec = (DWORD)rate;
    imp->wfx.wBitsPerSample = (WORD)bits;
    imp->wfx.nBlockAlign = imp->wfx.nChannels * imp->wfx.wBitsPerSample / 8;
    imp->wfx.nAvgBytesPerSec = imp->wfx.nSamplesPerSec * imp->wfx.nBlockAlign;
...
int winscap_run(WINSCAP_HANDLE handle, VOICE_BUFFER_CB cb, void *prv) {
    WAVEFORMATEX *wfxP = &(static_cast<struct WinscapImp *>(handle)->wfx);

    ensure(static_cast<struct WinscapImp *>(handle)->_init);
    ...
    for (;;) {
        ...
        HRESULT hrInit = pClient->Initialize(AUDCLNT_SHAREMODE_SHARED, AUDCLNT_STREAMFLAGS_LOOPBACK,
                                             16 * REFTIMES_PER_MILLISEC, 0, wfxP, nullptr);
        if (FAILED(hrInit)) {
        ...
        DWORD dwDelay = (DWORD)(((double)REFTIMES_PER_SEC * bufferFrameCount / wfxP->nSamplesPerSec) /
                                REFTIMES_PER_MILLISEC / 2);

        LPBYTE pSilence = (LPBYTE)malloc(wfxP->nBlockAlign * bufferFrameCount);
        EnsureFree freeSilence(pSilence);
        ZeroMemory(pSilence, wfxP->nBlockAlign * bufferFrameCount);
        ...
        while (!deviceChanged) {
            ...
            while (packetLength) {
                ...
                cb(static_cast<unsigned char *>(pData), wfxP->nBlockAlign * numFramesAvailable, prv);
